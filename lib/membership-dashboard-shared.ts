...
export const DEFAULT_MEMBERSHIP_DASHBOARD_LAYOUT: MembershipDashboardLayout = {
  ...
  visible: { birthdays: true, anniversaries: true, profiles: true, engagement: true, volunteer: true },
  widths: { birthdays: "half", anniversaries: "half", profiles: "half", engagement: "full", volunteer: "full" }
};
...
type ProfileFields = {
  ...
  family: { email: string | null; phone: string | null; addressStreet: string | null; addressCity: string | null; addressState: string | null; addressZip: string | null };
};
...
export function nextAnnualOccurrence(value: Date, today: Date) {
  ...
  const occurrence = new Date(Date.UTC(year, value.getUTCMonth(), value.getUTCDate()));
  if (occurrence < new Date(Date.UTC(year, today.getUTCMonth(), today.getUTCDate()))) occurrence.setUTCFullYear(year + 1);
  return occurrence;
}
...
export function periodComparison(current: number, prior: number) {
  ...
  return { current: safeCurrent, prior: safePrior, change, changePercent: safePrior === 0 ? null : Math.round((change / safePrior) * 100) };
}
...
export function normalizeDashboardLayout(input: unknown): MembershipDashboardLayout {
  ...
  const order = requestedOrder.filter((id, index): id is MembershipDashboardBlockId =>
    typeof id === "string" && MEMBERSHIP_DASHBOARD_BLOCK_IDS.includes(id as MembershipDashboardBlockId) && requestedOrder.indexOf(id) === index);
  for (const id of MEMBERSHIP_DASHBOARD_BLOCK_IDS) if (!order.includes(id)) order.push(id);
...
export type AnniversaryNameMember = { id: string; firstName: string; familyLastName: string; familyRoleSlug: string };

export function anniversaryDisplayName(members: AnniversaryNameMember[]) {
  const head = members.find((member) => member.familyRoleSlug === "head-of-household");
  const spouse = members.find((member) => member.familyRoleSlug === "spouse" && member.id !== head?.id);
  const primary = head ?? members[0];
  if (!primary) return "";
  return spouse ? `${primary.firstName} & ${spouse.firstName} ${primary.familyLastName}` : `${primary.firstName} ${primary.familyLastName}`;
}
