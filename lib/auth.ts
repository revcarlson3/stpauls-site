...
export function getCurrentUser() {
  return getServerSession(authOptions).then((session) => {
    ...
    return db.user.findUnique({ where: { id: session.user.id }, select: { isActive: true, group: { select: { permissions: { where: { permission: "ACCESS_ADMIN" }, select: { permission: true } } } } } }).then((membership) => {
