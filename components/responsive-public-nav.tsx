  ...
  return <>
    ...
    <div id="public-navigation" className={`${open ? "block" : "hidden"} absolute left-0 right-0 top-20 border-b border-ink/10 bg-sand p-5 lg:static lg:block lg:min-w-0 lg:flex-1 lg:overflow-visible lg:border-0 lg:bg-transparent lg:p-0`}>
      <nav aria-label="Primary navigation" className="text-sm font-medium lg:whitespace-nowrap">
        {items.length ? <MenuLinks items={items} className="grid gap-4 lg:flex lg:flex-nowrap lg:items-center lg:justify-end lg:gap-5" /> : <div className="flex flex-col gap-4 lg:flex-row lg:flex-nowrap lg:items-center lg:justify-end">{fallback}</div>}
