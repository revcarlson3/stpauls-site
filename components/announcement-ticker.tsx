...
export function AnnouncementTicker({ publicOnly = false }: { publicOnly?: boolean }) {
  ...
  return <div className={`w-full border-b text-sm ${barClasses}`}><div className="announcement-ticker relative w-full overflow-hidden py-2 pr-14 sm:py-3"><div className="announcement-ticker-track flex w-max items-center gap-5 px-4 sm:px-6 lg:px-8">{items.map((item) => <div key={item.id} className="flex max-w-[min(90vw,72rem)] shrink-0 items-center gap-3 overflow-hidden px-4 py-1"><strong className="shrink-0">{item.title}</strong><span className="truncate" dangerouslySetInnerHTML={{ __html: item.body }} /></div>)}</div>{!publicOnly && <div className={`absolute inset-y-0 right-0 z-10 flex items-center border-l border-black/10 px-3 shadow-[-8px_0_16px_rgba(0,0,0,0.08)] sm:px-5 ${barClasses}`}>{items.filter((item) => !item.acknowledged).map((item) => <button key={item.id} type="button" aria-label={`Dismiss ${item.title}`} title="Dismiss" className="focus-ring grid h-9 w-9 place-items-center rounded-full text-2xl leading-none opacity-70 hover:bg-black/10 hover:opacity-100" onClick={() => void acknowledge(item.id)}>×</button>)}</div>}</div></div>;
}
