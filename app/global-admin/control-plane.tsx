  ...
  return (
    <main className="min-h-screen bg-sand py-10 sm:py-14">
      <Container>
        ...
        {!overview && <div className="mt-8 grid gap-6 lg:grid-cols-2"><Card className="p-8"><h2 className="font-serif text-2xl">Global platform features</h2><p className="mt-2 text-sm leading-6 text-ink/60">These tools operate across the platform and do not require a site selection.</p><div className="mt-6 grid gap-3 sm:grid-cols-2"><Link href="/global-admin/announcements" className="focus-ring rounded-xl border border-ink/10 bg-white p-4 hover:border-coral"><p className="font-semibold">Announcements</p><p className="mt-1 text-xs text-ink/60">Publish and schedule notices across active sites.</p></Link><Link href="/global-admin/support" className="focus-ring rounded-xl border border-ink/10 bg-white p-4 hover:border-coral"><p className="font-semibold">Support management</p><p className="mt-1 text-xs text-ink/60">Review and respond to tickets from every site.</p></Link></div></Card><Card className="p-8"><h2 className="font-serif text-2xl">Site administration</h2><p className="mt-2 text-sm leading-6 text-ink/60">Select a site above to manage its identity, onboarding, domains, users, lifecycle, and operational overview.</p></Card></div>}
        {overview && (
          <div className="mt-8 grid gap-6">
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              ...
              <Card className="p-6 sm:p-8"><h2 className="font-serif text-2xl">Subscription and billing</h2>{overview.operational.subscription ? <><div className="mt-4 grid gap-3 sm:grid-cols-2"><Summary label="Plan" value={overview.operational.subscription.plan?.name ?? "Plan unavailable"} /><Summary label="Status" value={overview.operational.subscription.status} /><Summary label="Provider" value={overview.operational.subscription.provider} /><Summary label="Interval" value={overview.operational.subscription.interval} /></div><p className="mt-4 text-sm text-ink/60">{overview.operational.subscription.cancelAtPeriodEnd ? "Scheduled to cancel at period end." : "No billing changes are available in this overview."}</p></> : <p className="mt-2 text-sm text-ink/60">No subscription is recorded.</p>}</Card>
              <Card className="p-6 sm:p-8"><h2 className="font-serif text-2xl">Support activity</h2><div className="mt-4 grid gap-3 sm:grid-cols-2"><Summary label="Open tickets" value={String(overview.operational.support.openCount)} /><Summary label="Updated in 30 days" value={String(overview.operational.support.recentCount)} /></div>{overview.operational.support.recentTickets.length ? <div className="mt-4 grid gap-2">{overview.operational.support.recentTickets.map((ticket) => <div key={ticket.id} className="rounded-lg border border-ink/10 px-4 py-3"><p className="text-sm font-semibold">{ticket.subject}</p><p className="mt-1 text-xs text-ink/55">{ticket.status} · {ticket.priority} · {new Date(ticket.updatedAt).toLocaleDateString()}</p></div>)}</div> : <p className="mt-4 text-sm text-ink/60">No recent support tickets.</p>}<Link href="/global-admin/support" className="focus-ring mt-6 inline-block text-sm font-semibold text-coral hover:underline">Manage support tickets</Link></Card>
              <Card className="p-6 sm:p-8 lg:col-span-2"><h2 className="font-serif text-2xl">Global announcements</h2><div className="mt-4 grid gap-3 sm:grid-cols-2"><Summary label="Active for site" value={String(overview.operational.announcements.activeCount)} /><Summary label="Published in 30 days" value={String(overview.operational.announcements.recentCount)} /></div>{overview.operational.announcements.recent.length ? <div className="mt-4 grid gap-2">{overview.operational.announcements.recent.map((announcement) => <div key={announcement.id} className="rounded-lg border border-ink/10 px-4 py-3"><p className="text-sm font-semibold">{announcement.title}</p><p className="mt-1 text-xs text-ink/55">{announcement.severity} · {new Date(announcement.publishedAt).toLocaleDateString()} · {announcement.acknowledgedAt ? "Acknowledged" : "Not acknowledged"}</p></div>)}</div> : <p className="mt-4 text-sm text-ink/60">No recent global announcements.</p>}<Link href="/global-admin/announcements" className="focus-ring mt-6 inline-block text-sm font-semibold text-coral hover:underline">Manage global announcements</Link></Card>
            </div>
