...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error instanceof Error && (error.message === "Email delivery is not configured." || error.message === "Invalid registration details." || error.message === "No active church is available for registration." || error.message.startsWith("Password"))) {
