...
import { requestSelectedChurchUserPasswordReset } from "@/lib/global-admin-users";

export async function POST(request: Request) {
  const input = await request.json().catch(() => ({}));
  if (!input || typeof input.userId !== "string") return NextResponse.json({ error: "A user is required." }, { status: 400 });
  try {
    await requestSelectedChurchUserPasswordReset(input.userId);
    ...
    if (error instanceof Error && error.message.includes("Email delivery")) return NextResponse.json({ error: error.message }, { status: 503 });
    return NextResponse.json({ error: "Unable to send a password reset link." }, { status: 500 });
  }
}
