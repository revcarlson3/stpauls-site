...
export async function POST(request: Request) {
  try {
    ...
    if (!churchId) {
      ...
      if (context.church) await logAudit({ activityType: "global-admin-context-cleared", summary: "Cleared the selected site for bridge administration.", actorId: context.user.id, details: JSON.stringify({ boundary: "global-admin", selectedChurchId: context.church.id }) });
      return NextResponse.json({ church: null });
    }
    const church = churchId ? await db.church.findUnique({ where: { id: churchId }, select: { id: true, name: true, slug: true, status: true, lifecycleStatus: true, onboardingStatus: true } }) : null;
    if (!church) return NextResponse.json({ error: "Select an available site." }, { status: 400 });
