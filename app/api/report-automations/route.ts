...
export async function GET() {
  try {
    ...
    const automations = await db.reportAutomation.findMany({ where: { createdById: user.id }, orderBy: { updatedAt: "desc" }, include: { report: { select: { id: true, name: true, scope: true, reportType: true } }, runs: { orderBy: { createdAt: "desc" }, take: 5 } } });
...
export async function POST(request: Request) {
  try {
    ...
    const user = await requirePermission("ACCESS_ADMIN");
    const report = await db.membershipReport.findFirst({ where: { id: body.reportId, OR: [{ createdById: user.id }, { visibility: { in: ["MEMBERSHIP_MANAGERS", "EVENT_MANAGERS"] } }] } });
    ...
    const automation = await db.reportAutomation.create({ data: { ...body, schedule: body.schedule, criteria: body.criteria, recipients: body.recipients, inAppEnabled: body.inAppEnabled, nextRunAt: body.enabled ? nextScheduledAt(body.scheduleKind, body.schedule, body.timezone) : null, createdById: user.id } });
