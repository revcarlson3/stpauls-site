...
export async function GET() {
  try {
    await authorize();
    const workflows = await db.membershipWorkflow.findMany({ where: { isActive: true }, orderBy: { name: "asc" } });
...
export async function POST(request: Request) {
  try {
    ...
    const workflow = await db.membershipWorkflow.create({ data: { name, targetType, description: typeof input.description === "string" ? input.description.trim() || null : null, steps } });
