  ...
  try {
    ...
    const individual = await db.$transaction(async (transaction) => {
      const updated = await transaction.membershipIndividual.update({ where: { id: params.id }, data: {
        familyId: family.id, firstName: input.firstName.trim(), middleName: input.middleName?.trim() || null, lastName: input.lastName?.trim() || null, birthday, ageCategoryOverride: input.ageCategoryOverride?.trim() || null, gender: input.gender, maritalStatus: input.maritalStatus, status: input.status, memberTypeId: input.memberTypeId, familyRoleId: input.familyRoleId, email: input.email?.trim().toLowerCase() || null, emailMessagesAllowed: typeof input.emailMessagesAllowed === "boolean" ? input.emailMessagesAllowed : undefined, smsMessagesAllowed: typeof input.smsMessagesAllowed === "boolean" ? input.smsMessagesAllowed : undefined, cellphone: input.cellphone?.trim() || null, otherPhone: input.otherPhone?.trim() || null, otherPhoneType: input.otherPhoneType?.trim() || null, gradeLevel: input.gradeLevel?.trim() || null, weddingDate: input.weddingDate ? new Date(input.weddingDate) : null, deceasedDate: input.deceasedDate ? new Date(input.deceasedDate) : null
      } });
