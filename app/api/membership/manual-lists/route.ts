...
export async function GET() {
  try {
    ...
    const lists = await db.membershipManualList.findMany({ where: { churchId: scope.church.id }, orderBy: [{ name: "asc" }], select: { id: true, name: true, description: true, slug: true, _count: { select: { members: true } } } });
...
export async function POST(request: Request) {
  try {
    ...
    const source = copyOfId ? await db.membershipManualList.findFirst({ where: { id: copyOfId, churchId: scope.church.id }, select: { members: { select: { individualId: true } } } }) : null;
    if (copyOfId && !source) return NextResponse.json({ error: "Manual list to duplicate was not found." }, { status: 404 });
    const list = await db.membershipManualList.create({ data: { churchId: scope.church.id, name, description, slug: `${audienceSlug(name)}-${Date.now().toString(36)}`, ...(source ? { members: { create: source.members.map(({ individualId }) => ({ individualId })) } } : {}) }, select: { id: true, name: true, description: true, slug: true, _count: { select: { members: true } } } });
