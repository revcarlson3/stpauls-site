  ...
  try {
    ...
    const message = await db.membershipMessage.findUnique({
      ...
      select: {
        ...
        recipients: { where: { status: "FAILED" }, select: { id: true, individualId: true, address: true, displayName: true, individual: { select: { firstName: true, lastName: true, email: true, cellphone: true, emailMessagesAllowed: true, smsMessagesAllowed: true, doNotContact: true, status: true, family: { select: { lastName: true, addressCity: true } } } } } }
      }
    });
    ...
    const results = await Promise.all(message.recipients.map(async (recipient) => {
      const eligibility = membershipMessageEligibility(recipient.individual, message.channel);
      if (!eligibility.eligible || !eligibility.address) {
        await db.membershipMessageRecipient.update({
          where: { id: recipient.id },
          data: { status: "SKIPPED", eligibilityReason: eligibility.reason, failureReason: null }
        });
        return "excluded" as const;
      }
      const attemptedAt = new Date();
      await db.membershipMessageRecipient.update({ where: { id: recipient.id }, data: { address: eligibility.address, status: "PENDING", eligibilityReason: null, attemptCount: { increment: 1 }, lastAttemptAt: attemptedAt, failureReason: null } });
      try {
        ...
        if (message.channel === "EMAIL") await sendMembershipEmail({ recipient: eligibility.address, subject, bodyHtml, bodyText, attachments: message.attachments.filter((attachment): attachment is { fileName: string; contentBase64: string; mimeType: string } => Boolean(attachment.contentBase64)).map((attachment) => ({ fileName: attachment.fileName, contentBase64: attachment.contentBase64!, contentType: attachment.mimeType })) });
        else await sendSmsText(eligibility.address, bodyText);
