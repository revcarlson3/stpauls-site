...
export async function POST(request: Request) {
  ...
  try {
    return NextResponse.json(await createForm({ name: input.name.trim(), slug: input.slug, status: formStatus(input.status), enabled: input.enabled !== false, audience: input.audience === "MEMBERS" || input.audience === "GROUP" ? input.audience : "PUBLIC", audienceGroupIds: Array.isArray(input.audienceGroupIds) ? input.audienceGroupIds.filter((value): value is string => typeof value === "string") : [], definition: normalizeFormDefinition(input.definition), notificationSettings: asObject(input.notificationSettings), exportSettings: asObject(input.exportSettings) }), { status: 201 });
  } catch (error) {
