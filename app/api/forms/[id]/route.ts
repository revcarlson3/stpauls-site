...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    return NextResponse.json(await updateForm(params.id, { name: input.name.trim(), slug: input.slug, status: formStatus(input.status), enabled: input.enabled !== false, definition: normalizeFormDefinition(input.definition), notificationSettings: asObject(input.notificationSettings), exportSettings: asObject(input.exportSettings) }));
  } catch (error) {
