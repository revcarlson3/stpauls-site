...
export async function GET() {
  try {
    ...
    const [mail, code, current] = await Promise.all([getMailSettings(), getRegistrationCode(), db.securitySettings.findUnique({ where: { id: 1 }, select: { enabledModules: true, publicSiteEnabled: true, siteName: true, siteTagline: true, pollinationsApiKeyEncrypted: true, membershipMessageRecipientLimit: true, themeFamily: true, themeWidth: true, themeBackground: true, themeForeground: true, themeAccent: true, themeSurface: true, themeButtonPrimary: true, themeButtonSecondary: true, themeButtonDefault: true, themeNotificationPrimary: true, themeNotificationSecondary: true, themeNotificationDefault: true, themeNotificationSuccess: true, themeNotificationWarning: true, themeNotificationDanger: true, themeNotificationInfo: true, themeHeadingFont: true, themeBodyFont: true, themeRadius: true, themeElevation: true, themeButton: true, themeNav: true, themeSpacing: true, themeContentBorder: true } })]);
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(await db.securitySettings.upsert({
      where: { id: 1 },
      update: {
        siteName: input.siteName.trim(), siteTagline: input.siteTagline.trim(),
