...
export async function PATCH(request: Request) {
  ...
  if (!input || typeof input.name !== "string" || (input.currentPassword !== undefined && typeof input.currentPassword !== "string") || (input.newPassword !== undefined && typeof input.newPassword !== "string")) return NextResponse.json({ error: "Invalid account details." }, { status: 400 });
  try { return NextResponse.json(await updateOwnAccount(input)); }
  catch (error) {
    ...
    if (error instanceof Error && ["Name must", "Current password", "New password"].some((prefix) => error.message.startsWith(prefix))) return NextResponse.json({ error: error.message }, { status: 400 });
