...
export async function PATCH(request: Request) {
  ...
  const normalized = {
    ...
    mfaChallengePolicy: input?.mfaChallengePolicy,
    trustedDeviceDays: input?.trustedDeviceDays
  ...
  if (!input || typeof normalized.loginProtectionEnabled !== "boolean" || !Number.isInteger(normalized.maxFailedAttempts) || normalized.maxFailedAttempts < 1 || normalized.maxFailedAttempts > 20 || !Number.isInteger(normalized.lockoutMinutes) || normalized.lockoutMinutes < 1 || normalized.lockoutMinutes > 1440 || !["off", "challenge", "recaptcha-v3"].includes(normalized.captchaMode) || typeof normalized.emailMfaEnabled !== "boolean" || typeof normalized.smsMfaEnabled !== "boolean" || typeof normalized.authenticatorMfaEnabled !== "boolean" || !["twilio", "vonage", "aws-sns"].includes(normalized.smsProvider) || !["every-login", "trusted-device"].includes(normalized.mfaChallengePolicy) || !Number.isInteger(normalized.trustedDeviceDays) || normalized.trustedDeviceDays < 1 || normalized.trustedDeviceDays > 365 || !Number.isInteger(normalized.minPasswordLength) || normalized.minPasswordLength < 8 || normalized.minPasswordLength > 128 || ![normalized.passwordRequireUppercase, normalized.passwordRequireLowercase, normalized.passwordRequireNumber, normalized.passwordRequireSymbol, normalized.passwordExcludeAmbiguous].every((value) => typeof value === "boolean")) return NextResponse.json({ error: "Invalid security settings. Unused provider fields may be left blank." }, { status: 400 });
