...
export async function PATCH(request: Request) {
  ...
  if (!input || typeof input.loginProtectionEnabled !== "boolean" || !Number.isInteger(input.maxFailedAttempts) || input.maxFailedAttempts < 1 || input.maxFailedAttempts > 20 || !Number.isInteger(input.lockoutMinutes) || input.lockoutMinutes < 1 || input.lockoutMinutes > 1440 || !["off", "challenge", "recaptcha-v3"].includes(input.captchaMode) || typeof input.emailMfaEnabled !== "boolean" || typeof input.smsMfaEnabled !== "boolean" || typeof input.authenticatorMfaEnabled !== "boolean" || !["twilio", "vonage", "aws-sns"].includes(input.smsProvider) || typeof input.recaptchaSiteKey !== "string" || typeof input.recaptchaSecret !== "string" || typeof input.smsAccountId !== "string" || typeof input.smsAuthSecret !== "string" || typeof input.smsFrom !== "string" || typeof input.totpIssuer !== "string") return NextResponse.json({ error: "Invalid security settings." }, { status: 400 });
