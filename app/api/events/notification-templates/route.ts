...
async function getOrCreateTemplate(groupId: string | null) {
  const existing = await db.membershipEventNotificationTemplate.findFirst({ where: { groupId } });
  if (existing) return existing;
  const highest = await db.membershipEventNotificationTemplate.aggregate({ _max: { id: true } });
  return db.membershipEventNotificationTemplate.create({ data: { id: (highest._max.id ?? 0) + 1, groupId, ...defaults } });
}
...
export async function GET(request: Request) {
  try {
    ...
    if (groupId && !(await db.membershipVolunteerGroup.findUnique({ where: { id: groupId }, select: { id: true } }))) {
      return NextResponse.json({ error: "Volunteer group not found." }, { status: 404 });
    }
    const [template, groups] = await Promise.all([
      getOrCreateTemplate(groupId),
      db.membershipVolunteerGroup.findMany({ orderBy: { position: "asc" }, select: { id: true, name: true, singularName: true } })
    ]);
    return NextResponse.json({ template, groups });
  } catch {
    return NextResponse.json({ error: "Unable to load event notification templates." }, { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  try {
    ...
    if (groupId) {
      const group = await db.membershipVolunteerGroup.findUnique({ where: { id: groupId }, select: { id: true } });
    ...
    const template = existing
      ...
      : await db.membershipEventNotificationTemplate.create({ data: { id: ((await db.membershipEventNotificationTemplate.aggregate({ _max: { id: true } }))._max.id ?? 0) + 1, groupId, ...values } });
    return NextResponse.json({ template });
  } catch {
    return NextResponse.json({ error: "Unable to save event notification templates." }, { status: 500 });
  }
}
