...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  if (!input || typeof input.name !== "string" || input.name.trim().length < 2 || typeof input.email !== "string" || !input.email.includes("@") || typeof input.isActive !== "boolean" || (input.password !== undefined && (typeof input.password !== "string" || input.password.length < 1)) || (input.groupId !== null && typeof input.groupId !== "string")) {
    return NextResponse.json({ error: "Provide a valid name, email, group, and optional password." }, { status: 400 });
  }
