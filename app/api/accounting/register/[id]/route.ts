...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const existing = await db.accountingJournalEntry.findFirst({ where: { id: params.id, churchId: church.id }, select: { id: true, lines: { select: { accountId: true } } } });
    if (!existing) return NextResponse.json({ error: "Transaction not found." }, { status: 404 });
    const input = await request.json();
    const description = await ensureAccountingContact(church.id, input);
    if (description) input.description = description;
    const entry = await buildEntry(input, church.id, { allowedParentAccountIds: new Set(existing.lines.map((line) => line.accountId)) });
  ...
  } catch { return NextResponse.json({ error: "Unable to update the transaction." }, { status: 400 }); }
}

export async function DELETE(_request: Request, { params }: { params: { id: string } }) {
  try {
    const church = await authorize();
    const existing = await db.accountingJournalEntry.findFirst({ where: { id: params.id, churchId: church.id }, select: { id: true } });
    if (!existing) return NextResponse.json({ error: "Transaction not found." }, { status: 404 });
    await db.accountingJournalEntry.delete({ where: { id: existing.id } });
    return NextResponse.json({ ok: true });
  } catch { return NextResponse.json({ error: "Unable to delete the transaction." }, { status: 400 }); }
}
