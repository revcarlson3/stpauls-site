...
export async function GET() {
  try {
    ...
    const transactions = entries.map((entry) => {
      ...
      const amount =
        entry.entryType === "STANDARD" && bankLine
          ? Number(bankLine.debit) - Number(bankLine.credit)
          : Number(
              entry.lines.find((line) => Number(line.debit) > 0)?.debit ?? 0,
            );
      return {
        id: entry.id,
        ...
        status: entry.status,
        deletedGivingBatch: entry.deletedGivingBatch,
        deletedGivingBatchAt: entry.deletedGivingBatchAt?.toISOString() ?? null,
        amount: entry.entryType === "STANDARD" ? amount : Math.abs(amount),
        bankAccountId: entry.bankAccount?.id ?? bankLine?.bankAccount?.id ?? "",
        bankAccount:
          entry.bankAccount?.name ??
          bankLine?.bankAccount?.name ??
          "Unassigned",
          ...
          entry.entryType === "BANK_TRANSFER"
            ? (entry.lines.find(
                (line) =>
                  line.bankAccountId &&
                  line.bankAccountId !==
                    (entry.bankAccount?.id ?? bankLine?.bankAccount?.id),
              )?.bankAccount?.id ?? "")
            : "",
        category: offsetLine
          ? `${offsetLine.account.code} · ${offsetLine.account.name}`
          : entry.entryType === "ACCOUNT_TRANSFER"
            ? entry.lines
        ...
        accountId:
          entry.entryType === "ACCOUNT_TRANSFER"
            ? (entry.lines.find((line) => Number(line.credit) > 0)?.account
                .id ?? "")
            : (offsetLine?.account.id ?? ""),
        destinationAccountId:
          entry.entryType === "ACCOUNT_TRANSFER"
            ? (entry.lines.find((line) => Number(line.debit) > 0)?.account.id ??
              "")
            : "",
        fund: offsetLine?.fund?.name ?? null,
        fundId:
          entry.entryType === "FUND_TRANSFER"
            ? (entry.lines.find((line) => Number(line.credit) > 0)?.fund?.id ??
              "")
            : (offsetLine?.fund?.id ?? ""),
        destinationFundId:
          entry.entryType === "FUND_TRANSFER"
            ? (entry.lines.find((line) => Number(line.debit) > 0)?.fund?.id ??
              "")
            : "",
        allocations:
          entry.entryType === "STANDARD"
            ? entry.lines
                .filter((line) => !line.bankAccountId)
                .map((line) => ({
    ...
    console.error("Accounting register load failed", error);
    return NextResponse.json(
      { error: "Unable to load the register." },
      { status: 403 },
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(
      { error: "Unable to post the register entry." },
      { status: 400 },
    );
  }
}
