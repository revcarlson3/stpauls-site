...
export async function GET() {
  ...
  const groups = await db.securityGroup.findMany({ where: { churchId: context.church!.id }, orderBy: { name: "asc" }, select: { id: true, name: true } });
...
export async function POST(request: Request) {
  ...
  const group = await db.securityGroup.findFirst({ where: { id: groupId, churchId: context.church!.id }, select: { id: true } });
