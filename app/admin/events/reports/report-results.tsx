...
export function EventReportResults() {
  ...
  const loadReports = useCallback(async (preferredId?: string) => { const response = await fetch("/api/events/reports", { cache: "no-store" }); const value = await response.json(); const next = value.reports ?? []; setReports(next); const selected = next.find((item: Report) => item.id === preferredId) ?? next[0]; if (selected && !reportId) setReportId(selected.id); }, [reportId]);
  const run = useCallback(async (id: string, definition = preview) => {
  ...
  useEffect(() => { void loadReports(); const updated = () => void loadReports(); const previewHandler = (event: Event) => { const definition = (event as CustomEvent<Record<string, unknown>>).detail; setPreview(definition); void run("preview", definition); }; const runHandler = (event: Event) => { const id = (event as CustomEvent<{ id: string }>).detail.id; void run(id); }; window.addEventListener("event-reports-updated", updated); window.addEventListener("event-report-preview", previewHandler); window.addEventListener("event-report-run", runHandler); return () => { window.removeEventListener("event-reports-updated", updated); window.removeEventListener("event-report-preview", previewHandler); window.removeEventListener("event-report-run", runHandler); }; }, [loadReports, run]);
