        ...
        <div className="grid gap-2 rounded-xl border border-ink/10 p-3">
          {criteria.conditions.map((condition, index) => <div key={`${index}-${condition.field}`} className="grid gap-2 md:grid-cols-[1.2fr_1fr_1.2fr_auto]">
            ...
            {condition.operator === "isSet" || condition.operator === "isNotSet" ? <span className={`${inputClass} text-ink/55`}>No value needed</span>
              ...
              : condition.field === "doNotContact" ? <select value={condition.value} onChange={(event) => setCriteria({ ...criteria, conditions: criteria.conditions.map((row, rowIndex) => rowIndex === index ? { ...row, value: event.target.value } : row) })} className={inputClass} aria-label="Query value"><option value="">Choose...</option><option value="false">Contact allowed</option><option value="true">Do not contact</option></select>
              : condition.field === "preferredContactMethod" ? <select value={condition.value} onChange={(event) => setCriteria({ ...criteria, conditions: criteria.conditions.map((row, rowIndex) => rowIndex === index ? { ...row, value: event.target.value } : row) })} className={inputClass} aria-label="Query value"><option value="">Choose method...</option>{PREFERRED_CONTACT_METHOD_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
