...
export default function SiteSettingsPage() {
  const [settings, setSettings] = useState({ smtpHost: "", smtpPort: 587, smtpUser: "", smtpPassword: "", emailFrom: "", registrationCode: "", enabledModules: [] as string[] });
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    if (response.ok) setSettings((current) => ({ ...current, smtpPassword: "", registrationCode: "" }));
  }
  ...
  return (
    <main>
      <Container className="py-10 sm:py-14">
        ...
        <form onSubmit={(event) => void save(event)} className="mt-8 grid max-w-2xl gap-6">
          <section className="grid gap-4 rounded-2xl border border-ink/10 bg-white p-6 shadow-sm">
            <h2 className="font-serif text-2xl">Email and registration</h2>
            <p className="text-sm text-ink/60">SMTP and registration settings are stored server-side. Passwords and codes are encrypted with the application secret and never returned to the browser.</p>
            <label className="grid gap-1 text-sm font-semibold">SMTP host<input required value={settings.smtpHost} onChange={(event) => setSettings({ ...settings, smtpHost: event.target.value })} className="focus-ring rounded-lg border border-ink/15 px-3 py-2 font-normal" /></label>
            <label className="grid gap-1 text-sm font-semibold">SMTP port<input required type="number" value={settings.smtpPort} onChange={(event) => setSettings({ ...settings, smtpPort: Number(event.target.value) })} className="focus-ring rounded-lg border border-ink/15 px-3 py-2 font-normal" /></label>
            <label className="grid gap-1 text-sm font-semibold">SMTP username<input required value={settings.smtpUser} onChange={(event) => setSettings({ ...settings, smtpUser: event.target.value })} className="focus-ring rounded-lg border border-ink/15 px-3 py-2 font-normal" /></label>
            <label className="grid gap-1 text-sm font-semibold">SMTP password<input type="password" placeholder="Leave blank to keep current" value={settings.smtpPassword} onChange={(event) => setSettings({ ...settings, smtpPassword: event.target.value })} className="focus-ring rounded-lg border border-ink/15 px-3 py-2 font-normal" /></label>
          ...
          {canManageModules && <section className="grid gap-3 rounded-2xl border border-ink/10 bg-white p-6 shadow-sm">
          ...
          <div><button className="focus-ring w-fit rounded-full bg-coral px-5 py-3 text-sm font-semibold text-white" type="submit">Save settings</button>{message && <p role="status" className="mt-3 text-sm">{message}</p>}</div>
        </form>
      </Container>
    </main>
  );
