      ...
      <Container className="max-w-lg">
        <Card>
          ...
          <form className="mt-8 grid gap-4" onSubmit={handleSubmit}>
            ...
            {mfaChallenge && <><label className="grid gap-1 text-sm font-semibold">Authenticator or recovery code<input required inputMode="numeric" autoComplete="one-time-code" className="focus-ring rounded-lg border border-ink/15 px-3 py-2 font-normal" value={mfaCode} onChange={(event) => setMfaCode(event.target.value)} /></label><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={trustDevice} onChange={(event) => setTrustDevice(event.target.checked)} /> Trust this device when allowed by site policy</label></>}
